...
import { Plus, Search, Users as UsersIcon, ShieldCheck, UserX, Clock } from "lucide-react";
import { useState, useEffect } from "react";
import { useLocation } from "react-router-dom";

type StatusFilter = "all" | "active" | "inactive" | "suspended" | "hold";

const STATUS_TABS: { label: string; value: StatusFilter; color: string; active: string }[] = [
  { label: "All Users", value: "all", color: "text-slate-600", active: "bg-indigo-600 text-white shadow-md" },
  { label: "Active", value: "active", color: "text-emerald-600", active: "bg-emerald-500 text-white shadow-md" },
  { label: "Inactive", value: "inactive", color: "text-slate-500", active: "bg-slate-500 text-white shadow-md" },
  { label: "Suspended", value: "suspended", color: "text-red-500", active: "bg-red-500 text-white shadow-md" },
  { label: "Hold", value: "hold", color: "text-amber-600", active: "bg-amber-500 text-white shadow-md" },
];
...
export default function Users() {
  ...
  const { users, loading, error, refetch, unauthorized, deleteUser } = useAdminUsers();
    ...
    <>
      {unauthorized ? (
        ...
        <main className="min-h-screen p-6 bg-slate-50">
          ...
          <UserModal
            open={openModal}
            onClose={() => { setOpenModal(false); setSelectedUser(null); }}
            onSuccess={() => { setOpenModal(false); setSelectedUser(null); refetch(); }}
          />
          <UserEditModal
            open={openEditModal}
            user={selectedUser}
            onClose={() => { setOpenEditModal(false); setSelectedUser(null); }}
            onSuccess={() => { refetch(); setOpenEditModal(false); setSelectedUser(null); }}
          />
            ...
            <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-indigo-200 text-sm ml-1">
                  {users.length} total users &mdash; manage access, roles & status
                </p>
              </div>
