...
export default function UserEvents() {
  ...
  const hasFilters =
    selectedCategory || locationFilter || dateFrom || dateTo || minPrice || maxPrice || search;

  const categoryOptions = [
    { label: "All Events", value: "", emoji: "🎉" },
    { label: "Sports & Travel", value: "Sports & Travel", emoji: "⚽" },
    { label: "Science & Research", value: "Science & Research", emoji: "🔬" },
    { label: "New Years Eve", value: "New Years Eve", emoji: "🎆" },
    { label: "Industrial Eng.", value: "Industrial Engineering", emoji: "⚙️" },
    { label: "Holi", value: "Holi", emoji: "🎨" },
    { label: "Health & Wellness", value: "Health & Wellness", emoji: "🧘" },
    { label: "Garbe", value: "Garbe", emoji: "💃" },
    { label: "Public Event", value: "Public Event", emoji: "🏛️" },
  ];
  ...
  const handlePageClick = (p: number | string, index: number) => {
    if (typeof p === "number") {
      ...
      pageRefs.current[index]?.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" });
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (locationFilter.trim()) queryParams.append("location", locationFilter.trim());
        ...
        if (nextTotalPages > 0 && page > nextTotalPages) setPage(nextTotalPages);
      } catch (err) {
        if (!cancelled) { console.error(err); setEvents([]); }
      } finally {
  ...
  }, [page, selectedCategory, locationFilter, dateFrom, dateTo, minPrice, maxPrice, search]);
    ...
    <>
      ...
      <main className="pt-24 pb-20 grow bg-[#f8f9ff] min-h-screen">
        ...
        <section className="max-w-7xl mx-auto px-6 mb-10">
          <div className="relative rounded-[40px] overflow-hidden h-72 md:h-80 flex items-end p-10 group shadow-2xl shadow-violet-900/20">
            <img alt="Explore Events" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
              src="https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&w=1400&q=80" />
            <div className="absolute inset-0 bg-gradient-to-r from-violet-950/90 via-indigo-900/60 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/60 via-transparent to-transparent" />
            <div className="absolute top-0 right-0 w-64 h-64 bg-violet-400/20 rounded-full blur-3xl" />
            <div className="relative z-10 max-w-2xl">
              <nav className="flex items-center gap-2 text-white/60 font-semibold text-xs mb-4 uppercase tracking-widest">
                <span>Home</span><ChevronRight className="w-3 h-3" /><span className="text-white/90">Events</span>
              </nav>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-white/15 backdrop-blur-md rounded-full border border-white/20 mb-3">
                <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                <span className="text-white text-xs font-bold uppercase tracking-widest">
                  {total > 0 ? `${total} Events Live` : "Events Live"}
                </span>
              </div>
              <h1 className="text-white font-black text-4xl md:text-5xl leading-tight mb-3">Explore All Events</h1>
              <p className="text-white/75 text-base font-medium max-w-lg">Concerts, tech meetups, food festivals, sports — all in one place.</p>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 mb-8">
          <div className="flex flex-col md:flex-row gap-3 mb-5">
            <div className="search-box relative flex-1 bg-white border border-slate-200 rounded-2xl transition-all overflow-hidden flex items-center">
              ...
              {searchInput && (
                <button onClick={() => { setSearchInput(""); setPage(1); }} className="pr-4 text-slate-400 hover:text-slate-600">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
            <button onClick={() => setShowFilters((p) => !p)}
              ...
              {hasFilters && <span className="w-5 h-5 bg-white text-violet-600 rounded-full text-xs font-black flex items-center justify-center ml-1">!</span>}
            </button>
            {hasFilters && (
              <button onClick={clearFilters} className="flex items-center gap-2 px-5 py-3.5 rounded-2xl font-bold text-sm bg-red-50 text-red-500 border border-red-100 hover:bg-red-100 transition-all">
            ...
            <div className="filter-panel events-filter-glass rounded-3xl p-6 mb-5 shadow-xl border border-slate-100">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <div className="flex flex-col gap-2">
                  ...
                  <input value={locationInput} onChange={(e) => { setLocationInput(e.target.value); setPage(1); }}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input type="number" min={0} value={minPrice} onChange={(e) => { setMinPrice(e.target.value); setPage(1); }}
                      placeholder="Min ₹" className="w-full px-3 py-3 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-400 bg-white transition-all" />
                    <input type="number" min={0} value={maxPrice} onChange={(e) => { setMaxPrice(e.target.value); setPage(1); }}
                      placeholder="Max ₹" className="w-full px-3 py-3 rounded-xl border border-slate-200 text-sm font-medium text-slate-700 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-violet-200 focus:border-violet-400 bg-white transition-all" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {categoryOptions.map((cat) => {
              ...
              return (
                <button key={cat.value} onClick={() => { setSelectedCategory(cat.value); setPage(1); }}
          ...
          {!loading && (
            <p className="text-sm font-semibold text-slate-500 mt-4">
              {hasFilters
                ? <>Showing <span className="text-slate-900 font-bold">{events.length}</span> filtered results</>
                : <>Showing <span className="text-slate-900 font-bold">{events.length}</span> of <span className="text-slate-900 font-bold">{total}</span> events</>
              }
            </p>
          )}
        </section>
        ...
        <section className="max-w-7xl mx-auto px-6 min-h-96">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-28 text-center">
              ...
              <h3 className="text-2xl font-black text-slate-900 mb-3">No Events Found</h3>
              ...
              {hasFilters && (
                <button onClick={clearFilters} className="px-8 py-3.5 bg-violet-600 text-white rounded-2xl font-bold shadow-lg shadow-violet-600/25 hover:bg-violet-700 hover:-translate-y-0.5 transition-all">
                  Clear All Filters
                </button>
              )}
            </div>
          ...
          {!loading && paginationTotal > 1 && (
            <div className="flex items-center justify-center gap-2 mt-14 flex-wrap">
              <button onClick={() => { setPage(1); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === 1}
                className="page-btn w-10 h-10 flex items-center justify-center font-black rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-violet-400 hover:text-violet-600 disabled:opacity-30 disabled:cursor-not-allowed">«</button>
              <button onClick={() => { setPage((p) => Math.max(p - 1, 1)); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === 1}
                className="page-btn w-10 h-10 flex items-center justify-center font-black rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-violet-400 hover:text-violet-600 disabled:opacity-30 disabled:cursor-not-allowed">‹</button>
              {getPagination(page, paginationTotal).map((p, i) =>
                p === "..." ? (
                  <span key={i} className="w-10 h-10 flex items-center justify-center text-slate-400 font-bold">…</span>
                ) : (
                  <button ref={(el) => { pageRefs.current[i] = el; }} key={i} onClick={() => handlePageClick(p, i)}
              ...
              <button onClick={() => { setPage((p) => Math.min(p + 1, paginationTotal)); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === paginationTotal}
                className="page-btn w-10 h-10 flex items-center justify-center font-black rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-violet-400 hover:text-violet-600 disabled:opacity-30 disabled:cursor-not-allowed">›</button>
              <button onClick={() => { setPage(paginationTotal); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === paginationTotal}
                className="page-btn w-10 h-10 flex items-center justify-center font-black rounded-xl border border-slate-200 bg-white text-slate-600 hover:border-violet-400 hover:text-violet-600 disabled:opacity-30 disabled:cursor-not-allowed">»</button>
              <span className="text-xs font-bold text-slate-400 px-3 py-2 bg-white rounded-xl border border-slate-100 ml-2">Page {page} of {paginationTotal}</span>
            </div>
          )}
        </section>
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-12 p-12 md:p-16">
              <div className="flex-1">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/15 backdrop-blur-sm rounded-full border border-white/20 mb-6">
                  ...
                  <span className="text-white text-xs font-bold uppercase tracking-widest">Exclusive Member Benefits</span>
                </div>
                ...
                <p className="text-violet-200 text-base font-medium max-w-md mb-8 leading-relaxed">
                  Early event access, artist meet-and-greets, and 15% off all workshop tickets — forever.
                </p>
                <div className="flex flex-wrap gap-4">
                  <button className="group flex items-center gap-2 bg-white text-violet-700 px-8 py-3.5 rounded-2xl font-bold shadow-xl hover:-translate-y-0.5 transition-all">
                    Join Community <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </button>
                ...
                <div className="flex flex-wrap gap-3 mt-6">
                  {["Early Bird Access", "15% Discount", "Exclusive Meet-ups", "Priority Support"].map((perk) => (
                    <span key={perk} className="flex items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full text-white/80 text-xs font-semibold">
              ...
              <div className="relative z-10 flex-shrink-0 hidden md:block">
                ...
                <div className="absolute -bottom-3 -right-3 bg-white rounded-2xl px-4 py-2 shadow-xl flex items-center gap-2">
                  ...
                  <span className="text-xs font-black text-slate-900">50K+ Members</span>
                </div>
              </div>
            </div>
          </div>
