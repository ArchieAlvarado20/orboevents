  ...
  const filteredTickets = tickets
    ...
    .filter((t) => {
      ...
      return (
        ...
        t.eventId?.location?.toLowerCase().includes(q)
      );
    });
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f8f9ff] pb-24">
        <div className="max-w-7xl mx-auto px-6 py-10 mt-16">
          ...
          {/* ── Tabs + Search + Filter ── */}
          <div
            className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 fade-up"
            ...
            <div className="space-y-5">
              {filteredTickets.map((ticket, idx) => {
                ...
                return (
                  <div
                    key={ticket._id}
                    className={`ticket-card fade-up bg-white rounded-3xl overflow-hidden border border-slate-100 flex flex-col md:flex-row`}
                  ...
                  >
                    ...
                    <div className="md:w-56 relative overflow-hidden shrink-0 h-52 md:h-auto">
                      ...
                      <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm rounded-2xl px-3 py-2 shadow-md text-center">
                        ...
                        <p className="text-xl font-black text-violet-600 leading-none">
                          {new Date(ticket.slotId?.date || "").getDate()}
                        </p>
                      </div>
                      {/* Status badge */}
                      <div className="absolute bottom-3 left-3">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wide border ${cfg.classes}`}
                        >
                        ...
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xl font-black text-violet-600">
                              ₹{ticket.ticketTypeId?.price?.toFixed(2) ?? "—"}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-5 pt-5 border-t border-slate-100">
                        ...
                        {ticket.status === "used" && (
                          <span className="flex items-center gap-1.5 text-slate-400 text-xs font-bold ml-1">
                            <XCircle className="w-4 h-4" /> This ticket has been
                            used
                          </span>
                        )}
                      </div>
                    ...
                    {/* Right accent */}
                    <div
                      className={`w-2 shrink-0 ${ticket.status === "active" ? "bg-gradient-to-b from-violet-600 to-purple-700" : "bg-slate-200"}`}
                    />
                  </div>
                );
              })}
              ...
              <div className="relative bg-gradient-to-br from-violet-950 via-violet-800 to-indigo-900 rounded-3xl p-10 md:p-14 overflow-hidden shadow-2xl shadow-violet-900/20">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                  <div>
                    ...
                    <p className="text-violet-200 font-medium max-w-md">
                      Browse thousands of upcoming events and book in seconds.
                    </p>
                  </div>
